#!/bin/sh
# demo.sh — see agent-jail working without any manual setup.
#
...
# the verdicts land.
#
#   sudo ~/agent-jail-fresh/scripts/demo.sh           # JAILED — escapes show "blocked"
#   sudo ~/agent-jail-fresh/scripts/demo.sh --audit   # UNCONFINED — escapes show "LEAKED"
#
# Press q to quit the watcher. The real tool is scripts/agent-jail; this just
...
PROJ=${DEMO_DIR:-/tmp/agent-jail-demo}
...
DECOY_SECRET=/tmp/agent-jail-demo-secret.txt
...
WORKLOAD=/tmp/agent-jail-demo-workload
cat > "$WORKLOAD.sh" <<EOF
n=0
...
# "Text file busy".
STANDIN_DIR=$(mktemp -d /tmp/agent-jail-demo.XXXXXX)
OMP_STANDIN="$STANDIN_DIR/omp"
# Clear any stale standins/processes from earlier demo runs.
pkill -9 -f "/tmp/agent-jail-demo\." 2>/dev/null || true
